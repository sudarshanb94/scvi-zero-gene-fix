"""
Reference: https://github.com/snap-stanford/GEARS/blob/master/gears/model.py
"""
    ...
    def forward(self, x, pert_idx, batch):
        ...
        if self.no_perturb:
            ...
            for idx, layer in enumerate(self.layers_emb_pos):
                pos_emb = layer(pos_emb, self.G_coexpress.to(x.device), self.G_coexpress_weight.to(x.device))
            ...
            for idx, i in enumerate(pert_idx):
                if i != -1:
                    pert_index.append([idx, i])
            
            pert_index = torch.tensor(pert_index).T.to(x.device)
            
            pert_global_emb = self.pert_emb(
                torch.LongTensor(list(range(self.num_perts))).to(x.device)
            ) # (num_perts, hidden_size)

            if self.args["pert_type"] == "genetic":
                ## augment global perturbation embedding with GNN
                for idx, layer in enumerate(self.sim_layers):
                    pert_global_emb = layer(pert_global_emb, self.G_sim.to(x.device), self.G_sim_weight.to(x.device))
